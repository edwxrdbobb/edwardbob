  ...
  return (
    ...
    >
      ...
      >
        {items.map((item, idx) => (
          <li
            className="w-[200px] max-w-full relative rounded-lg border border-border flex-shrink-0 bg-card px-8 py-6 md:w-[250px]"
            style={{
                ...
                {item.logo ? (
                  <div className="relative h-16 w-full flex-shrink-0">
                    <img
                      src={item.logo}
                      alt={item.name}
                      className="h-full w-full rounded-md object-contain rounded-md filter grayscale hover:grayscale-1 transition-all duration-300"
                    />
                  </div>
                ) : null}
                <span className="flex flex-col gap-1">
                  {/* <span className="text-sm leading-[1.6] text-foreground font-normal">{item.name}</span> */}
                  {item.title && (
